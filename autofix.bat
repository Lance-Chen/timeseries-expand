...
echo       Running mypy...
python -m mypy src
if errorlevel 1 (echo FAIL: mypy& exit /b 1)
echo       OK.

echo [2/4] Commit...
git add . >nul 2>&1
git diff --cached --quiet >nul 2>&1 && (echo       Nothing to commit.& goto :push) || (
    git commit -m "fix: release.yml, mypy compat, .gitignore hardening" >nul 2>&1
...
echo [4/4] Done.
echo CI: https://github.com/Lance-Chen/timeseries-expand/actions
endlocal